...
async def main():
    ...
    modules = [
        {
            ...
            "runner": lambda: DynamicOrchestrator(
                ...
                api_inventory=[{"api": {"endpoint": "/identity/api/v2/user/{id}", "method": "GET"}}],
        ...
        },
        {
            ...
            "runner": lambda: BOPLAOrchestrator(ba_discovery.Config(output=ba_discovery.OutputConfig(path=output_dir))).run_assessment(
                repo_path=repo_path,
                openapi_spec=openapi_spec,
        ...
        },
        {
            ...
            "runner": lambda: run_broken_authentication(repo_path, openapi_spec, runtime_traffic, output_dir),
            "is_async": True
        },
        {
    ...
    for mod in modules:
        ...
        try:
            ...
            if isinstance(findings, list):
                ...
                findings_count = findings.get("vulnerabilities_detected", 0) or len(findings.get("findings", []))
    ...
    print("="*90)
    print(f"{'MODULE NAME':<45} | {'DIRECTORY':<35} | {'TIME (s)':<10} | {'STATUS':<10} | {'FINDINGS':<8}")
    print("-" * 120)
    for res in results:
        time_str = f"{res['time']:.3f}"
        print(f"{res['name']:<45} | {res['dir']:<35} | {time_str:<10} | {res['status'][:10]:<10} | {res['findings']:<8}")
    print("="*90 + "\n")
