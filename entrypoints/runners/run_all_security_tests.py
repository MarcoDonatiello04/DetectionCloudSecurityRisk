...
def parse_args():
    ...
    parser.add_argument("--keycloak-url", default="http://localhost:8080", help="Keycloak IDP base URL")
    ...
    parser.add_argument("--assessment-mode", action="store_true", help="Run in Assessment Mode (skip Keycloak/seeding, infer from traffic)")
...
def load_openapi_spec(repo_path: str):
    openapi_spec = None
    openapi_paths = [
        Path(repo_path) / "fixtures/api_vulnerabilities" / "openapi.yaml",
    ...
    for path in traffic_paths:
        if path.is_file():
            try:
                ...
                logger.info(f"Loaded runtime traffic data from {path} ({len(runtime_traffic)} records)")
                break
            except Exception as e:
                ...
                if method in path_item:
                    ...
                    if key not in seen:
                        ...
                        api_inventory.append({
                            "api": {"endpoint": path, "method": method.upper()}
                        })
        ...
        for entry in runtime_traffic:
            ...
            if path:
                ...
                if key not in seen:
                    seen.add(key)
                    api_inventory.append({
                        "api": {"endpoint": path, "method": method}
                    })
...
async def run_broken_authentication(args, openapi_spec, runtime_traffic, target_reachable: bool):
    ...
    try:
        ...
        logger.warning(f"[BA] LLM Stack Discovery unavailable ({e}). Using Python/FastAPI fallback.")
    ...
    if target_reachable:
        try:
            async with httpx.AsyncClient(base_url=args.target_url, timeout=config.scanner.timeout_http) as client:
                tester = ba_dynamic_tester.DynamicTester(config, client=client, auth_intel=auth_intel)
...
def run_bopla(args, openapi_spec, runtime_traffic):
    ...
    if not args.assessment_mode:
        try:
            ...
            logger.warning(f"[BOPLA] Keycloak IdentityManager unavailable ({e}). Using traffic/mock fallback.")
...
def print_unified_summary(ba_results, bopla_data, bola_findings):
    ...
    if bola_findings is not None:
        vulnerable = sum(1 for f in bola_findings if f.validation_status.value == "CONFIRMED" and f.rule_id != "dynamic-test-secure")
...
async def main_async():
    ...
    ba_results = []
    try:
        ba_results = await run_broken_authentication(args, openapi_spec, runtime_traffic, target_reachable)
    except Exception as e:
