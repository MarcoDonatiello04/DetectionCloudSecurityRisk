# Broken Object Property Level Authorization (BOPLA) Security Report
- **Data Scansione**: 2026-07-18T15:40:56.084338
- **Target Application**: http://localhost:5000
- **Repository**: DetectionCloudSecurityRisk

## Property Authorization Resilience Score
- **Punteggio**: 100/100
- **Livello di Rischio**: **LOW**

## Sommario Esecuzione
- Oggetti Scoperti: 8
- Proprietà Totali: 16
- Proprietà Protette Rilevate: 0
- Test Dinamici Eseguiti: 0
- Vulnerabilità Confermate (FAIL): 0

## Risultati dei Test Dinamici
| Test ID | Proprietà | Endpoint | Metodo | Risultato | Confidenza |
|---|---|---|---|---|---|

## Dettaglio Findings Rilevati
Nessuna vulnerabilità BOPLA rilevata con successo.
