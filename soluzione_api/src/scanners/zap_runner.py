...
class ZAPRunner(ScannerInterface):
    """
    ...
    def scan(self, target_dir: str) -> List[Any]:
        # Prova a caricare l'URL target da .target_env nel workspace
        target_url = "http://localhost:5000"
        env_file = os.path.join(target_dir, "config/environments/.target_env")
                ...
                with open(env_file, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        if line.startswith("TARGET_URL="):
                            target_url = line.split("=", 1)[1]
            except Exception:
                pass
        return self.scan_targets([target_url])
