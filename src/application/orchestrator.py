...
from src.domain.interfaces import IScanner, IDetector, IEventBus
...
class ScanPipelineOrchestrator:
    ...
    def _on_finding_detected(self, event: Any) -> None:
        ...
        if isinstance(finding, Finding):
            ...
            logger.info(f"📥 Ricevuto Finding via Event Bus: [{finding.severity.value}] {finding.title} ({finding.finding_id})")

    def run_pipeline(self, static_scanners: List[IScanner], raw_traffic_data: List[Dict[str, Any]] = None) -> List[Finding]:
        """
        ...
        for scanner in static_scanners:
            try:
                ...
                logger.info(f"   - Scanner {scanner.__class__.__name__} ha rilevato {len(findings)} findings.")
            except Exception as e:
                logger.error(f"   - Fallimento dello scanner {scanner.__class__.__name__}: {e}", exc_info=True)
        ...
        self.event_bus.publish(EVENT_STATIC_SCAN_COMPLETED, {
        ...
        if raw_traffic_data:
            logger.info(f"📢 [Fase 3] Pubblicazione EVENT_TRAFFIC_CAPTURED con {len(raw_traffic_data)} record di traffico...")
            self.event_bus.publish(EVENT_TRAFFIC_CAPTURED, {
                "traffic": raw_traffic_data,
                "target_dir": self.target_dir
            })
        ...
        for f in self.detected_findings:
            if f.source in (FindingSource.RUNTIME_VALIDATOR, FindingSource.SHADOW_API, FindingSource.ZAP_DAST):
        ...
        self.event_bus.publish(EVENT_PIPELINE_COMPLETED, {
    ...
    def _register_detector_handlers(self, detectors: List[IDetector]) -> None:
        ...
        for detector in detectors:
            ...
            events = getattr(detector, "subscribed_events", [EVENT_STATIC_SCAN_COMPLETED, EVENT_TRAFFIC_CAPTURED])
            ...
            for event_type in events:
                ...
                def make_handler(det=detector, ev_type=event_type):
                    ...
                    handler.__name__ = f"handler_{det.__class__.__name__}_{ev_type.replace('.', '_')}"
                    return handler
