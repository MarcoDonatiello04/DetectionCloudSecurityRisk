...
def generate_live_api_traffic(target_host: str):
    ...
    # Estraiamo l'host e la porta dal target_host per ricostruire la richiesta
    parsed_target = urllib.parse.urlparse(target_host)
    target_netloc = parsed_target.netloc # es: localhost:4566
    target_path_prefix = parsed_target.path.rstrip('/') # es: /restapis/0uuofdetq8/dev/_user_request_
    ...
    for ep in endpoints_to_call:
        ...
        full_path = f"{target_path_prefix}{path}"
        if query:
            full_path += f"?{query}"
            
        proxy_absolute_url = f"http://{target_netloc}{full_path}"
        
        # Connessione diretta a Mitmproxy
        try:
            ...
            body_data = None
            if ep.get("body"):
                body_data = json.dumps(ep["body"])
                headers["Content-Type"] = "application/json"
                
            conn.request(method, proxy_absolute_url, body=body_data, headers=headers)
            response = conn.getresponse()
            status = response.status
            response.read() # Consumiamo la risposta per liberare la connessione
            conn.close()
