...
def analyze_openapi(spec: dict, enrich_spec: bool = False) -> list[SsrfFinding]:
    """
    ...
    for path, path_item in paths.items():
        ...
        for method, operation in path_item.items():
            ...
            body_findings = _check_request_body(operation.get("requestBody", {}), path, method)
...
def _check_url_parameter(param: dict, path: str, method: str, spec: dict) -> SsrfFinding | None:
...
def _check_request_body(request_body: dict, path: str, method: str) -> list[SsrfFinding]:
    ...
    for _mime_type, media_type in content.items():
