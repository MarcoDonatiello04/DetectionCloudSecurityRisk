            ...
            if isinstance(node.value, ast.Call):
                if isinstance(node.value.func, ast.Attribute) and node.value.func.attr == "json":
                    ...
                    if isinstance(node.value.func.value, ast.Call):
                        ...
                        if isinstance(subcall.func, ast.Attribute) and subcall.func.attr in (
                            "get",
                            "post",
                            "put",
                            "patch",
                            ...
                            if (
                                isinstance(subcall.func.value, ast.Name)
                                and subcall.func.value.id in ("requests", "httpx")
                                and subcall.args
                            ):
                    ...
                    node.value.func.value, ast.Call
                ):
                    subcall = node.value.func.value
                    if isinstance(subcall.func, ast.Attribute) and subcall.func.attr in (
                        "get",
                        ...
                        if isinstance(subcall.func.value, ast.Name) and subcall.func.value.id in (
                            "requests",
                            "httpx",
                        ):
                            is_json_call = True
            ...
            if is_json_call:
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        self.json_vars[target.id] = {"url": url_str, "lineno": node.lineno}
            ...
            if isinstance(node.value, ast.Call):
                ...
                if node.value.keywords:
                    for kw in node.value.keywords:
                        if (isinstance(kw.value, ast.Name) and kw.value.id in self.json_vars) or (
                            kw.arg is None
                            and isinstance(kw.value, ast.Name)
                            and kw.value.id in self.json_vars
                        ):
        ...
        def visit_Call(self, node: ast.Call):
            ...
            if is_db_execute and node.args:
                ...
                if contains_unvalidated:
                    ...
                    url_val = (
                        self.json_vars[target_var]["url"] if target_var in self.json_vars else None
                    )
                    ...
                    if (
                        ...
                        if kw.value.id not in self.validated_vars:
                            evidence_line = (
                                ...
                                or "Model.objects.create(**data)"
                            )
                            url_val = (
                                self.json_vars[kw.value.id]["url"]
                                if kw.value.id in self.json_vars
                                else None
                            )
                            self.findings.append(
                                UnsafeConsumptionFinding(
