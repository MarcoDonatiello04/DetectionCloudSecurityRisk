...
Percorso: src/core/bola/attack_vector.py
...
class ContextAwareAttackGenerator:
    ...
    def execute_tampering(
        ...
        sub_alice = self.extract_sub_from_jwt(headers_matrix.get("userA", {}).get("Authorization")) or uuid_alice
        sub_bob = self.extract_sub_from_jwt(headers_matrix.get("userB", {}).get("Authorization")) or uuid_bob
        sub_charlie = self.extract_sub_from_jwt(headers_matrix.get("userC", {}).get("Authorization")) or uuid_charlie

        logger.info(f"🔑 [CONTEXT-AWARE SEEDING] Claim sub decodificati - Alice: {sub_alice}, Bob: {sub_bob}, Admin: {sub_charlie}")
