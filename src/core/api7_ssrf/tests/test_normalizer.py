...
def test_normalize_maps_severity():
    semgrep_json = {"results": [{
        ...
        "extra": {
            ...
            "metadata": {"rule_id_internal": "SS-001", "cwe": "CWE-918", "category": "direct_url_from_input"}
        }
    }]}
...
def test_compute_confidence():
    ...
    assert _compute_confidence({
        "extra": {
            "metadata": {"category": "cloud_metadata_access"}
        }
    }) == 0.70
...
def test_detect_allow_redirects():
    assert _detect_allow_redirects({"extra": {"lines": "requests.get(url, allow_redirects=True)"}}) is True
    assert _detect_allow_redirects({"extra": {"lines": "requests.get(url, allow_redirects=False)"}}) is False
    assert _detect_allow_redirects({"check_id": "ssrf-python-requests-redirect-following", "extra": {"lines": ""}}) is True
