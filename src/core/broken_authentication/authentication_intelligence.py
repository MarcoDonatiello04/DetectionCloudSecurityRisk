...
def _extract_path_from_route(route_str: str) -> Optional[str]:
    ...
    if route_str.startswith("/") or "/" in route_str:
        ...
        if not any(kw in cleaned for kw in ["def ", "function ", "func ", "class ", "@", "app.", "router."]):
            return cleaned
    return None
...
class AuthenticationIntelligenceEngine:
    ...
    def correlate(
        ...
        if any(kw in "".join(auth_libs) for kw in ["jwt", "jose", "oauth", "auth0", "keycloak", "cognito", "okta"]):
            auth_type = "JWT"
        ...
        for file_score in ast_output:
            ...
            for func, role in file_score.auth_decorators.items():
                ...
                if matched_path:
                    ...
                    protected_endpoints[func] = {"source": "AST_decorator", "required_roles": [role] if role else []}
            ...
            for entry in runtime_traffic:
                ...
                if not token:
                    ...
                    if cookie_header:
                        ...
                        token_match = re.search(r'(?:access_token|token|session|jwt)=([^;]+)', str(cookie_header), re.IGNORECASE)
                        if token_match:
                ...
                if token:
                    ...
                    if claims:
                        ...
                        # Extract roles
                        for role_key in ["role", "roles", "groups", "realm_access", "resource_access"]:
                        ...
                        for perm_key in ["permissions", "permission", "scope", "scopes"]:
                            if perm_key in claims:
                                ...
                                if isinstance(val, list):
                                    ...
                                    permissions_set.update(v.strip() for v in val.split(" ") if v.strip())

        # 5. Identity Provider Intelligence
        if identity_provider:
            idp_name = identity_provider.lower().strip()
            logger.info(f"Rilevato Identity Provider: {identity_provider}. Popolamento metadati OIDC...")
            ...
            for rel_file in discovery_output.file_configurazione_rilevanti:
                ...
                if file_path.is_file():
                    try:
                        ...
                        # Extract ACS URL
                        acs_match = re.search(r'(?:acs_url|assertionConsumerServiceUrl|acsUrl)["\']?\s*[:=]\s*["\']([^"\']+)["\']', content, re.I)
                        if acs_match:
                            saml_acs_url = acs_match.group(1)
                        # Extract Entity ID
                        entity_match = re.search(r'(?:entity_id|entityId|issuer|entityID)["\']?\s*[:=]\s*["\']([^"\']+)["\']', content, re.I)
                        if entity_match:
                            saml_entity_id = entity_match.group(1)
                        # Extract Certificates
                        cert_matches = re.findall(r'-----BEGIN CERTIFICATE-----\s*([^-]+)\s*-----END CERTIFICATE-----', content)
        ...
        non_jwt_detected = set(discovery_output.non_jwt_mechanisms if hasattr(discovery_output, "non_jwt_mechanisms") else [])
        ...
        for file_score in ast_output:
            ...
            if any(k in all_file_signals for k in ["jwks", "jwk_client", "pyjwkclient", "jwksclient", "jwk-client"]):
                jwks_signals = True
            ...
            if any(k in all_file_signals for k in ["code_challenge", "code_verifier", "pkce"]):
                pkce_signals = True
            if any(k in all_file_signals for k in ["rotation", "rotate", "revoke", "reuse_detection", "invalidat"]):
                if "refresh" in all_file_signals:
                    refresh_rotation_signals = True

            if "basic" in all_file_signals:
                non_jwt_detected.add("Basic Auth")
            if "api_key" in all_file_signals or "apikey" in all_file_signals or "x-api-key" in all_file_signals:
                non_jwt_detected.add("API Key")
            if any(k in all_file_signals for k in ["client.crt", "client.key", "mtls", "mutual_tls", "client-cert", "client_cert"]):
        ...
        if openapi_spec:
            ...
            if security_schemes:
                for scheme_name, scheme in security_schemes.items():
                    scheme_type = scheme.get("type", "").lower()
                    if scheme_type == "apikey" or scheme.get("name", "").lower() in ("x-api-key", "api_key", "apikey"):
                        non_jwt_detected.add("API Key")
        ...
        if getattr(discovery_output, "discovery_methods", None):
            heuristic_count = sum(1 for v in discovery_output.discovery_methods.values() if v == "heuristic")
        ...
        graph = AuthenticationKnowledgeGraph(
            ...
            jwt_claims=sorted(list(jwt_claims_set)),
            roles=sorted(list(roles_set)),
            permissions=sorted(list(permissions_set)),
            auth_middlewares=sorted(list(auth_middlewares_set)),
            protected_endpoints=protected_endpoints,
            auth_functions=sorted(list(auth_functions_set)),
