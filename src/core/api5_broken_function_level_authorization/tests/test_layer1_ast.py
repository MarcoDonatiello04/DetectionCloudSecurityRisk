import tempfile
import pathlib
from src.core.api5_broken_function_level_authorization.layers.layer1_ast import analyze_ast
...
def update_role(user_id):
    user.role = "admin"
'''
    from src.core.api5_broken_function_level_authorization import detector
    report = detector.analyze_content(content)
    findings_on_endpoint = [
        f for f in report.findings
        if f.endpoint and "/users/" in f.endpoint
    ]
...
'''
    ...
    findings_on_endpoint = [
        f for f in report.findings
        if f.endpoint and "/debug/" in f.endpoint
    ]
    rule_ids = {f.rule_id for f in findings_on_endpoint}
