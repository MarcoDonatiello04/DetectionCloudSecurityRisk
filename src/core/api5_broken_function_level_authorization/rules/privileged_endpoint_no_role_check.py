...
PRIVILEGED_PATH_PATTERNS = {
    "/admin", "/admins", "/internal", "/management",
    "/debug", "/config", "/settings/admin",
    "/users/*/role", "/users/*/promote", "/users/*/ban",
    "/export", "/reports/all", "/metrics/internal"
}
...
class PrivilegedEndpointNoRoleCheckRule:
            ...
            for method in route_methods:
                if is_privileged_path(route_path, method):
                    ...
                    if not has_role_check:
                        ...
                        confidence = 0.75 if method in ("DELETE", "PUT") and not any(p in route_path for p in ("/admin", "/internal", "/management", "/debug", "/config")) else 0.95
    ...
    def analyze_javascript(root: Node, file_path: str) -> list[FunctionAuthzFinding]:
        ...
        for call in calls:
            ...
            if not args_node or len(args_node.children) < 3: # 1st arg is path, last is handler, should have at least path and handler
                continue
                
            path_node = args_node.children[1] # standard tree-sitter javascript: argument list starts with '(' then arguments
            ...
            if is_privileged_path(route_path, method):
                ...
                has_role_check = False
                middleware_args = args_node.children[2:-2] # skip path (1st) and handler (last), and parenthesis
