...
from src.core.api5_broken_function_level_authorization.rules.privileged_endpoint_no_role_check import (
    _node_text, _collect_nodes, _parse_python_decorator, ROLE_CHECK_DECORATORS, has_inline_role_check
)
from src.core.api5_broken_function_level_authorization.rules.auth_without_authz import AUTH_ONLY_DECORATORS
        ...
        for dec_def in dec_defs:
            ...
            if is_shadow_path(full_path):
                if not has_role_check:
                    severity = "MEDIUM" if has_auth and any(x in full_path.lower() for x in ("debug", "test")) else "HIGH"
    ...
    def analyze_javascript(root: Node, file_path: str) -> list[FunctionAuthzFinding]:
        ...
        for call in calls:
            ...
            if is_shadow_path(route_path):
                if not has_role_check:
                    severity = "MEDIUM" if has_auth and any(x in route_path.lower() for x in ("debug", "test")) else "HIGH"
                    findings.append(FunctionAuthzFinding(
                        rule_id=ShadowAdminFunctionRule.rule_id,
                        cwe_id=ShadowAdminFunctionRule.cwe_id,
                        category=ShadowAdminFunctionRule.category,
