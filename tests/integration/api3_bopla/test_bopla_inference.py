...
def test_is_auth_expression():
    ...
    assert (
        PropertyAuthorizationInferenceEngine._is_auth_expression("user.role == 'manager'") is True
    )
...
def update_profile(self):
    if current_user.is_admin:
        x = user.email
"""
    discovered = {"User": {"salary", "email", "name"}}
    results = {"User": {"salary": [], "email": [], "name": []}}
    PropertyAuthorizationInferenceEngine._scan_text_for_auth_regex(
        content, ".py", discovered, results
    )
...
def test_documentation_correlation():
    ...
    assert (
        ...
        in salary_ev.documentation_issues
    )
...
""",
    ...
    res = PropertyAuthorizationInferenceEngine.build_cross_model_occurrences(
        str(tmp_path), "python", discovered
    )
