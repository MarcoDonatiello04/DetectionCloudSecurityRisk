...
GROUND_TRUTH = {
    "vulnerable_app": {
        "expected_findings": [
            {"rule_id": "SC-001", "must_find": True},
            {"rule_id": "SC-002", "must_find": True},
            {"rule_id": "SC-003", "must_find": True},
            {"rule_id": "SC-004", "must_find": True},
...
def run_validation(crapi_path: str | None = None):
    ...
    for app_name, ground_truth in GROUND_TRUTH.items():
        fixture_path = PROJECT_ROOT / "data/test_targets" / "security_misconfiguration" / app_name
